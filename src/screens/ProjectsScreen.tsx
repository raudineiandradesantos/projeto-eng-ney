...
export default function ProjectsScreen() {
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        <Text style={s.screenTitle}>Projetos</Text>
        {currentProject && (
          <View style={s.activeBadge}>
            <Text style={s.activeBadgeText}>Ativo: {currentProject.name}</Text>
          </View>
        )}
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={s.empty}>Nenhum projeto. Crie o primeiro!</Text>}
          ListFooterComponent={hasMore ? (
            <TouchableOpacity style={s.loadMore} onPress={() => setPage(p => p + 1)}>
              <Text style={s.loadMoreText}>Carregar mais ({projects.length - paged.length} restantes)</Text>
            </TouchableOpacity>
          ) : null}
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                ...
                <View style={s.cardHeader}>
                  <Text style={[s.cardName, isActive && s.cardNameActive]}>{item.name}</Text>
                  {isActive && (
                    <View style={s.activeDot}>
                      <Text style={s.activeDotText}>ATIVO</Text>
                    </View>
                  )}
                </View>
                ...
                {item.client ? <Text style={s.clientText}>Cliente: {item.client}</Text> : null}
                {item.address ? <Text style={s.addressText}>{item.address}</Text> : null}
              </TouchableOpacity>
            );
          }}
        />
          ...
          <View style={s.modalCard}>
            ...
            <View style={s.modalBtns}>
              ...
              <TouchableOpacity style={s.saveBtn} onPress={createProject} disabled={saving}>
                {saving ? <ActivityIndicator color={Colors.bg} /> : <Text style={s.saveBtnText}>Criar</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  activeBadge: { backgroundColor: 'rgba(0,212,255,0.12)', borderWidth: 1, borderColor: Colors.cyan, borderRadius: 6, paddingHorizontal: 10, paddingVertical: 4, alignSelf: 'flex-start' },
  ...
  card: { backgroundColor: Colors.bg1, borderWidth: 1, borderColor: Colors.line, borderRadius: 10, padding: 14, marginBottom: 10 },
  ...
  modalCard: { backgroundColor: Colors.bg1, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 40, borderTopWidth: 1, borderTopColor: Colors.line, maxHeight: '85%' },
  modalTitle: { fontSize: 18, fontWeight: '700', color: Colors.ink, marginBottom: 16 },
  fieldLabel: { fontSize: 12, color: Colors.ink2, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 },
  input: { backgroundColor: Colors.bg2, borderWidth: 1, borderColor: Colors.line, borderRadius: 8, padding: 12, color: Colors.ink, marginBottom: 14, fontSize: 14 },
