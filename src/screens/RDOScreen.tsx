...
export default function RDOScreen() {
  ...
  const loadRDO = useCallback(async () => {
    ...
    try {
      ...
      // Non-critical: silently skip if no RDO exists for the selected date
    }
    setRefreshing(false);
  ...
  async function saveRDO() {
    ...
    try {
      ...
      if (rdoId) {
        await setDoc(doc(db, 'rdos', rdoId), data, { merge: true });
      } else {
        const ref = await addDoc(collection(db, 'rdos'), data);
        setRdoId(ref.id);
      }
    ...
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          ...
          onRefresh={() => { setRefreshing(true); loadRDO(); }}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={s.chips}>
            {WEATHER.map(w => (
              <TouchableOpacity
                key={w}
                style={[s.chip, rdo.weather === w && s.chipActive]}
...
const s = StyleSheet.create({
  ...
  card: { backgroundColor: Colors.bg1, borderWidth: 1, borderColor: Colors.line, borderRadius: 12, padding: 16, marginBottom: 12 },
  cardTitle: { fontSize: 11, fontWeight: '600', color: Colors.ink2, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 12 },
  input: { backgroundColor: Colors.bg2, borderWidth: 1, borderColor: Colors.line, borderRadius: 8, padding: 12, color: Colors.ink, marginBottom: 8, fontSize: 14 },
  textarea: { height: 100 },
  ...
  actItem: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: Colors.line },
  ...
  mainBtn: { backgroundColor: Colors.cyan, borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 8 },
