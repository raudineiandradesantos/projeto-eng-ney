...
export default function FotosScreen() {
  ...
  const loadPhotos = useCallback(async () => {
    if (!docId) return;
    try {
      const snap = await getDoc(doc(db, 'photos', docId));
  ...
  async function pickPhoto(fromCamera: boolean) {
    ...
    // canceled is the correct property name in expo-image-picker
...
const s = StyleSheet.create({
  ...
  loadingBar: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingBottom: 8 },
  loadingText: { color: Colors.ink2, fontSize: 13 },
  grid: { padding: 8, paddingBottom: 100 },
  photoCard: { margin: 4, backgroundColor: Colors.bg1, borderWidth: 1, borderColor: Colors.line, borderRadius: 10, overflow: 'hidden' },
  ...
  photoModal: { backgroundColor: Colors.bg1, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: Colors.line },
  fullPhoto: { width: '100%', height: 260, borderRadius: 10, marginBottom: 14, backgroundColor: Colors.bg2 },
  captionInput: { backgroundColor: Colors.bg2, borderWidth: 1, borderColor: Colors.line, borderRadius: 8, padding: 12, color: Colors.ink, marginBottom: 12, fontSize: 14 },
  modalBtns: { flexDirection: 'row', gap: 10, marginBottom: 10 },
  saveCaption: { flex: 1, backgroundColor: Colors.cyan, borderRadius: 8, padding: 12, alignItems: 'center' },
  saveCaptionText: { color: Colors.bg, fontWeight: '700', fontSize: 13 },
  deleteCaptionBtn: { flex: 1, borderWidth: 1, borderColor: '#ff3b3b', borderRadius: 8, padding: 12, alignItems: 'center' },
