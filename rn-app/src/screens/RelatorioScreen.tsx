  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [checkSnap, photoSnap] = await Promise.all([
        firestore().collection('checklists').doc(docId).get(),
        firestore().collection('photos').doc(docId).get(),
      ]);
        ...
        <View style={s.checkCard}>
          ...
          <View style={s.progressBar}>
            <View style={[s.progressFill, {
              width: checkItems.length > 0 ? (doneChecks / checkItems.length * 100) + '%' : '0%'
            }]} />
          </View>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  checkCard: {backgroundColor: Colors.bg1, borderWidth: 1, borderColor: Colors.line,
  ...
  cardTitle: {fontSize: 11, color: Colors.ink2, fontWeight: '600',
    textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 10},
  actionRow: {flexDirection: 'row', gap: 10, marginBottom: 16},
  shareBtn: {flex: 1, backgroundColor: Colors.cyan, borderRadius: 8,
  ...
  annCard: {backgroundColor: Colors.bg1, borderWidth: 1, borderColor: Colors.line,
    borderRadius: 10, padding: 12, marginBottom: 8},
  annCategory: {fontSize: 10, color: Colors.cyan, fontWeight: '600',
