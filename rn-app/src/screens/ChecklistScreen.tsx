...
export default function ChecklistScreen() {
  ...
  const loadChecklist = useCallback(async () => {
    if (!docId) return;
    try {
      const snap = await firestore().collection('checklists').doc(docId).get();
      if (snap.exists()) {
  ...
  async function save(list: CheckItem[]) {
    if (!docId) return;
    try {
      await firestore().collection('checklists').doc(docId).set({
        items: list,
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);loadChecklist();}} tintColor={Colors.cyan}/>}>
      ...
      {showAdd && (
        <View style={s.addForm}>
          <TextInput style={s.input} value={newItem} onChangeText={setNewItem}
            placeholder="Novo item..." placeholderTextColor={Colors.ink3}
            autoFocus />
...
const s = StyleSheet.create({
  ...
  progressCard:{backgroundColor:Colors.bg1,borderWidth:1,borderColor:Colors.line,
  ...
  addForm:{backgroundColor:Colors.bg2,borderRadius:10,padding:12,marginBottom:12,
    borderWidth:1,borderColor:Colors.line},
  input:{backgroundColor:Colors.bg3,borderWidth:1,borderColor:Colors.line,
  ...
  addItemBtn:{borderWidth:1,borderColor:Colors.line,borderRadius:10,padding:14,
